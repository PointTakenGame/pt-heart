...
//
// Nathan, 2026-09-05 (playtest findings 16 and 17): "the boss writes essentially
// the same point three times" and "are they actually reading what the user
// writes?". Both were true, and both were this file rather than the plumbing.
// The runner does pass the player's last sentence into the model, but /api/coach
// is not served by `npm run dev`, so every line in a local playtest is the
// fallback below. Two things follow. Topic-agnostic never required her three
// clean turns to be the *same* argument in three costumes, so they are now three
// different moves: a tradeoff claim, then a precedent claim, with the summary in
// between doing neither. And a fallback summary can quote the player verbatim,
// which is topic-agnostic by construction and is the one thing that answers
// "is she reading me?" with a yes you can see on the screen.
//
// Nathan's ruling of 2026-09-05 on how to pay for the rest: authored variants per
// topic, and the player picks one of three rather than typing their own. A closed
// list is what makes per-topic authoring finite, so her lines can be about the
// actual thing instead of about "the cost of this".
//
// BALANCE LEDGER for this level, and it is load-bearing. Round 1 opens on Sofia,
// so she speaks before the player has revealed a side. Authored per-topic lines
 ...
 *
 *  Closed list, and the player picks from it rather than typing (Nathan,
 *  2026-09-05). A free-text topic meant every authored line had to work for every
 *  possible subject, which is what made them read like nothing. */
...
export interface Turn {
   ...
   *  since `npm run dev` does not serve /api/coach. A function is handed the
   *  player's last sentence and the chosen topic, so a fallback can play the
   *  player back instead of inventing a position for them, and can be about the
   *  actual subject instead of about "this". Use `byTopic` for the common case. */
...
export const TURNS: Turn[] = [
  ...
  },
  {
    ...
    kind: 'speak',
    foul: null,
    // The precedent move. Different shape from her opener on purpose (Nathan,
    // finding 17: "she writes essentially the same point three times"). Still
    // sideless — she objects to settling it this way, not to either answer.
    fallback: byTopic({
      loans:
 ...
 * anything are about nothing — which is how findings 16 and 17 happened.
 */
