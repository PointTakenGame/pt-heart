...
**Status markers**, inline on every specific value: `[ruled]` Steve or a registry decision established it. `[unratified]` it exists in the shipped code and nobody ruled it, with file and line. `[vibecoded]` the author of this document invented it because a sketch is easier to correct than a blank; a proposal, not canon. `GAP:` refused to invent, a human must answer. Section 10 collects the gaps.
...
- **A tie:** the print deck does not address one. The online edition calls equal tokens a draw `[unratified]` (`game/src/showdown.ts:517-522`). GAP-1.
- **A full game** is three listen-and-summarize rounds, then one Final Showdown round. Roles swap between games: page 1 ends "Next game: Swap Roles!" `[ruled]`
...
**Contested calls.** Ruled 2026-08-15 (`HEART-T260815-09`): when the Referee flags a foul, the wronged player casts the deciding vote on whether it landed. The accused may contest by playing a rule card, which brings a second opinion, and the wronged player still decides after hearing it. `[ruled]` The print deck does not print this procedure. GAP-3.
...
3. **Player A answers the question.** They say what was missed, or that nothing was. This is the ground truth for Fake Listening: the person who was summarized rules on whether they were heard. `[ruled]` No timer is printed for this beat. Provisional build default: fifteen seconds, one correction pass. `[vibecoded]`, not ruled, see GAP-4.
...
Page 2's instruction, verbatim: "Referee scoring: Decide when Player A receives (or gives) 🙏's". Three outcomes per step: "... follows the rules", which moves nothing and is printed "(expected, no 🙏's)"; "... earns a humility bonus!", where the other player pays the speaker; "... is Naughty", where the speaker pays the other player. **The size of a bonus or a naughty penalty is not printed. GAP-5.** Provisional build default: a bonus moves one token, and a naughty step costs what the matching foul costs during the rounds, two for a Judging-shaped fail and one otherwise. `[vibecoded]`, not ruled, and a player would notice if it changed.
...
5. **Ask, on an unclear call, rather than rule.** The pattern is "I heard 'obviously'. Was that contested for you?", put to the possibly-wronged player, not to the speaker. Their answer is the ruling. `[ruled]` How often the Referee may interrupt a live argument, and whether an ambiguous hit goes to the listener mid-turn or waits for the turn boundary, is explicitly unanswered. GAP-6.
...
- Judging moves two, Opinions as Facts one, Fake Listening one per missing major point, so a badly incomplete summary can cost more than a Judging. `[ruled]` Provisional build default: at most three missing points charged on any one summary, so a single weak turn cannot empty a purse. `[vibecoded]`, not ruled, see GAP-8.
...
- Most tokens at the end wins. Equal is a draw online `[unratified]` (`game/src/showdown.ts:517-522`); print is silent, GAP-1.
...
2. **A teaching ladder in front of the game.** Five levels, ruled 2026-08-15 (`HEART-T260815-27`): L1 the word "You" (Judging), L2 In my head, because (Opinions as Facts), L3 Did I miss anything? (Fake Listening), L4 Full Showdown with all three fouls live, L5 the Final Showdown. `[ruled]` L1 to L3 are built, L4 ships as a three-round match against the AI, L5 is the least-built part of the game. **The numbering itself is contested; do not build against it.** `roadmap.md` carries this as an open GAP: a later brief describes the Final Showdown's steps as "levels 4 to 6ish", which fits neither this five-level ladder nor the built L4. Renumber nothing until it is answered; the ruling request is filed as `BIZ-T260828-02`. An unnumbered agreement screen sits before L1. Boss names are explicitly placeholders and the docs and the code disagree on them, so quote none of them anywhere player facing. **None of this vocabulary is player facing in print:** the v7 deck contains zero level, phase, boss, tier, stage, or chapter words. Never source a level count from the deck.
...
5. **Attempt ceilings.** Three attempts on an answering step, then the game moves on `[unratified]` (`game/src/engine.ts:486`, `game/src/showdown.ts:433`). Print has no attempt limit.
...
7. **No timers.** The thirty and forty-five second clocks are not implemented anywhere in the shipped code; online turns are untimed today.
...
Every item here is a question a human must answer. None has been invented above.

GAP-1: What happens on a tie? Print says only "Most 🙏 at end wins", and the online edition calls equal tokens a draw. Is a draw the print rule too, or is there a tiebreak, for example counting the Final Showdown bonuses separately?

GAP-2: Does the Referee, Player C, score anything at all? The deck never says. Related and also unanswered: page 1 says "Next game: Swap Roles!" and glosses it only with icons, so it is unclear whether swapping means A and B trade sides of the topic or a disputant trades seats with the Referee, and how many games make a session.

GAP-3: The contested-call procedure (wronged player rules, accused may contest with a rule card, wronged player still decides) is ruled for online but printed nowhere in the v7 deck. Does the print game use it, and if so what physical object is the rule card?

GAP-4: How long does the summarized player get to answer "did I miss anything?", and may they correct more than once? An earlier design pass proposed a one-correction maximum; it was never ruled.

GAP-5: How large is a Final Showdown humility bonus, and how large is a naughty penalty? Page 2 shows direction arrows but no token count. Is a bonus one token, and does a naughty step cost the same as the equivalent foul during the rounds, so a Judging-shaped "#humility-fail" costs two?

GAP-6: How often may the Referee interrupt a live argument with a flag-then-ask question before players mute it, and does an ambiguous hit go to the listener mid-turn or wait for the turn boundary? Explicitly unanswered as of the 2026-08-10 pass.

GAP-7: Is a Judging foul chargeable during the Final Showdown in addition to the step being scored naughty? The two sharpest naughty examples on page 2 are Judging sentences, and the deck does not say whether the card also gets played.

GAP-8: Fake Listening is priced per missing major point. What counts as a major point, and who decides? The summarized player is the obvious answer and the deck implies it, but with no threshold a hostile Referee can price one weak summary at four tokens.

GAP-9: Is there any foul the summarizing player can commit on a speaking turn, or the speaker on a summary turn? The online edition restricts which cards are callable by turn kind; print does not say.

GAP-10: When does Opinions as Facts fire on a bare declarative with no smoke alarm phrase, for example "that policy will raise costs" said flatly? The card's own incorrect example is exactly that shape, but the shipped detector deliberately does not catch it, conceding it is the larger half of the foul as the card defines it. A human Referee needs a stated threshold.

GAP-11: Does a foul committed while stating a *positive* value in Final Showdown step 3 count as Judging? "You value fairness" is the bonus and "you don't care about low-income people" is the fail, and the line between a generous characterization and a verdict about a person is undefined.

GAP-12: Earlier design work deferred five rule questions that no v7 mechanic replaces, and they stay open: what threshold makes a non-canonical certainty phrase ("It's not complicated", "We know what happened", "that's just how it works") an Opinions as Facts foul; whether motive-questioning is a foul in a professional context; whether adding a missed point to a summary is repair or a new contribution; whether letting a disputed call favor the speaker is exploitable; and whether the mandatory summary step is too awkward to sustain early, since one pass observed it naturalizing only around the third or fourth round.
