...
- Zero tokens ends nothing. `[ruled, Nathan; Q11]` A purse can empty and the player climbs back; purses clamp at zero, never go negative, and always sum to fourteen. This retired the earlier "instant loss at zero".
...
- Most tokens at the end wins `[ruled]`, and equal tokens is a draw `[ruled, Nathan]`, in print as well as online (`src/showdown.ts`). Section 1 carries it.

**No fractional tokens.** `[ruled, Nathan; Q9]` The earlier ruling (Steve, 2026-08-24) that a missed whistle costs half a token, rendered "½", is retired. Online, a player who fails to whistle a foul the coach saw now moves nothing — the offendee simply keeps the token the whistle would have taken. Tokens are whole integers everywhere (`src/showdown.ts`). Print never had half tokens or a missed-call penalty, because in print the Referee does the calling.
...
**Same in both:** the roles, three cards, three fouls, the same penalties, seven tokens each, transfer not destruction, the two-step summary, the two-step opinion frame, three rounds with a flip inside each, the Final Showdown's three steps and rubric, most tokens wins.
...
2. **A teaching ladder in front of the game. Seven levels, player-first** `[ruled, Nathan]`: L1 Judging (player seat), L2 Opinions as Facts (player), L3 Fake Listening (player), L4 the referee seat with the full round and all three cards live, L5 the Showdown (player) — the base game is complete here, L6 the Final Showdown in the referee seat *(deferred)*, L7 the Final Showdown as player *(deferred)*. L1 to L5 are built in this rebuild; 6 and 7 are an add-on and the game is complete and playable without them. **L1 to L3 withhold the referee entirely** — the word, the role and the three-player structure are all held back until L4, which opens by telling the player they learn as ref first and play it themselves next. This supersedes both the five-level ladder ruled 2026-08-15 (`HEART-T260815-27`), which ended at L5 and had no referee level, and the interim six-level scheme. **Save by slug, never by number:** `about-the-argument`, `my-opinion-not-a-fact`, `the-summary-gate`, `in-the-ref-seat`, `the-showdown`. An unnumbered agreement screen sits before L1. Boss names are placeholders and are not player facing; the code is authoritative (Victor, Olivia, Noemi, Sofia), so quote none of them anywhere player facing. **None of this vocabulary is player facing in print:** the v7 deck contains zero level, phase, boss, tier, stage, or chapter words. Never source a level count from the deck.
3. **The player, not a referee, throws the whistle at the AI.** The human calls fouls on the AI and the coach calls fouls on the human. Missing one of the AI's fouls now moves nothing `[ruled, Nathan; Q9]` — the earlier half-token penalty is retired; whistling a clean line costs a full token `[unratified]` (`src/showdown.ts`). Both exist only because there is no third human.
4. **Wrong answers cost tokens during teaching.** In L1 to L3 a wrong answer costs one token; a non-answer — thin text or an unchanged prefill — reopens the step for free. There are no retries: the step does not loop `[ruled, Nathan; Q12]`. Both purses run from seven from L1 on `[unratified]` (`src/engine.ts`). Print has no quiz and no equivalent.
5. **No attempt ceilings.** `[ruled, Nathan; Q12]` The earlier three-attempt cap on an answering step is retired: there is no cap on corrections, a wrong answer costs a token and play moves on, and a non-answer reopens the step free. This matches print, which has no attempt limit either. A summary-coverage check for Fake Listening is still unbuilt; `roadmap.md` section 7 carries that build item.
